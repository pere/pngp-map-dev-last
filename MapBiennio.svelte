<script>
	...
	import { mapBiennioMarkerCardShared as markShared } from './mapMarkerCardShared.svelte.js';
	...
	import { _chartStoresMapWithBiennio as _chartStoresMapWithBiennio } from '../chartStores.svelte.js';
	...
	let localShowStatsArray = $state(['Richness','Abundance','Shannon']);
	watch(()=>localShowStatsArray,(curr,prev)=>{
		if (!prev || prev === curr) return;
		console.log('localShowStatsArray changed', curr, prev);

		//chartStoresMap.setPlotsParams({showStatsArray:curr});
	});
	...
	let loadedInitialSource = $state(false);
	let loaded = $state(false);
	let mapReady = $derived(
		(d) => map && map.getStyle && loaded && map.getLayer('plotGeodata') && loadedInitialSource
	);

	watch(()=>loadedInitialSource,(curr,prev)=>{
		console.log('loadedInitialSource changed', curr, prev);
	});
	let markerVisible = $state(false);
	let markerTitleVisible = $state(true);
	// `map` is not $state — depend on load flags so we re-query markers after mount/load
	watch(() => [markerVisible, selTaxonShort, loaded, loadedInitialSource], ([markerVisible, selTaxonShort], prev) => {
...
return false
	...
	watch(()=>_chartStoresMapWithBiennio.mouseoverPlot,(curr,prev)=>{
		if (!prev || prev === curr) return;
		console.log('chartStoresMap.mouseoverPlot changed', curr, prev);
	...
	watch(()=>selTaxonShort,(curr,prev)=>{
		...
		_chartStoresMapWithBiennio.setPlotsParams({taxons:[names.taxons.find((d)=>d.value===curr)]});
		_chartStoresMapWithBiennio._updateKey++;
	});
	...
	_chartStoresMapWithBiennio.setPlotsParams({activeChartsTab:'charts_geo'});
	_chartStoresMapWithBiennio.setPlotsParams({plots:plots});
	_chartStoresMapWithBiennio.setPlotsParams({taxons:[names.taxons.find((d)=>d.value===selTaxon)]});

	_chartStoresMapWithBiennio.setPlotsParams({byBiennio:true});
	...
	function updatePointPositions() {
		const m = map;
		const plotSrc = m?.getSource?.('plotGeodata_source');
		const plots = derivedGroupedPlots;
		if (!m || !plotSrc || !plots?.length) return;
		...
		const updatedFeatures = plots.map((feature) => {
...
let allReady=$derived.by(()=>{
	console.warn('mapReady', mapReady);
	console.warn('map', map);
	console.warn('map?.getSource?.("plotGeodata_source")', map?.getSource?.('plotGeodata_source'));
	return mapReady && loadedInitialSource
});
	...
	$effect(() => {
		...
		console.log('bieennio map2', map, derivedGroupedPlots);
			...
			map.on('load', () => {
				...
				geoPlotsByValley.toprights.forEach((d, index) => {
					...
					requestAnimationFrame(() => {
						...
						const markerInDom = mapContainer?.querySelector(
							`#marker-${d.id}-${type}`
						);
						if (markerInDom) {
...
{#if geoPlotsByValley?.toprights?.length > 0}
	...
	<div class="w-full h-full relative">
		...
		<div class="absolute top-2 left-4 z-11">
		...
		bind:this={mapContainer}>
			...
			{/if}
			<div
				class="taxaManager w-auto absolute top-15 left-4 bg-gray-100  shadow-lg z-10 p-3"
			>
				<h4 class="text-sm  mb-2 capitalize">
					{type} - {selTaxon}
				</h4>

				<Select.Root
					type="single"
					class="w-auto overflow-none  rounded-sm"
					bind:value={selTaxon}
					onValueChange={(value) => {
						// keyComponent.push(type+'_'+value);
					}}
				>
					<Select.Trigger data-tid="labels.choose_taxon" class="pl-1 border border-gray-300 rounded-sm">
	...
	{#if showStats}
	<div class="absolute bottom-4 right-20 z-10 w-[90vw] bg-amber-700 ">

{#key _chartStoresMapWithBiennio._updateKey}

			<PlotsContainer fromMap={true} byBiennio={true} />
{/key}
	</div>
	{/if}
{:else}{/if}

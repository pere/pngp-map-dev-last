...
let { newSpeciesYearsList } = $props();

// $inspect('newSpeciesYearsList', newSpeciesYearsList) // Disabled for performance
...
let foundTaxes=$derived.by(() => {
    return newSpeciesYearsList.map((item,i) => {
...
let reduced=$derived.by(() => {
    const result = newSpeciesYearsList.reduce((acc, item) => {
        let tax_id=item.tax_id;
        let year=item.year;
        /* let color=foundTaxes.find((d) => d.tax_id === tax_id).color; */
        if (!acc.categories.includes(year)) {
    ...
    // Sort categories (years) in ascending order
    result.categories.sort((a, b) => a - b);

    return result;
})

    // $inspect('foundTaxes', foundTaxes) // Disabled for performance
    let highchartsYearsBarConfigModified = $derived.by(() => {
        let series=reduced.data.map((d) => {
            let f=foundTaxes.find((v) => v.tax_id === d.tax_id);
            let color=f.color;

            // Sort data points to match the sorted categories order
            const sortedData = d.data.sort((a, b) => a.category - b.category);
...
{#if (highchartsYearsBarConfigModified)}
<div class="border-pngp border rounded-md pt-0 pb-0 p-2">
    <div class="w-full">
        <div use:chartAction={highchartsYearsBarConfigModified} class="w-full h-50"></div>
    </div>
 <div class="w-full">

...
let showStats = $state(false);
	...
	watch(()=>selTaxonShort,(curr,prev)=>{
		...
		_chartStoresMapWithoutBiennio._updateKey++;
	});
	...
	_chartStoresMapWithoutBiennio.setPlotsParams({byBiennio:false});
...
{#if geoPlotsByValley?.toprights?.length > 0}
	...
	<div class="w-full h-full relative">
		...
		>
			...
			/* 			_chartStoresMapWithoutBiennio.plotsParams={..._chartStoresMapWithoutBiennio.plotsParams, taxons: [value]};
						console.warn(_chartStoresMapWithoutBiennio.plotsParams); */
	...
	<!-- using w-30vw crashes charts! -->
	 {#if showStats}
	<div class="{showStats ? 'block' : 'hidden'} absolute bottom-4 right-20 z-10 w-[95vw] bg-amber-700">
		{#key _chartStoresMapWithoutBiennio._updateKey}
		<!-- {JSON.stringify(_chartStoresMapWithoutBiennio.plotsParams)} -->
			<PlotsContainer fromMap={true} byBiennio={false} />
		{/key}
		</div>
	{/if}

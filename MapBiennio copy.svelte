...
return false
	...
	watch(()=>chartStoresMap.mouseoverPlot,(curr,prev)=>{
		if (!prev || prev === curr) return;
	...
	watch(()=>selTaxonShort,(curr,prev)=>{
		...
		chartStoresMap.setPlotsParams({taxons:[names.taxons.find((d)=>d.value===curr)]});
	});
	...
	chartStoresMap.setPlotsParams({activeChartsTab:'charts_geo'});
	chartStoresMap.setPlotsParams({plots:plots});
	chartStoresMap.setPlotsParams({taxons:[names.taxons.find((d)=>d.value===selTaxon)]});

	chartStoresMap.setPlotsParams({byBiennio:markShared._interface==='withBiennio'?true:false});
...
{#if geoPlotsByValley?.toprights?.length > 0}
	...
	{#if showStats}
	<div class="absolute bottom-4 right-4 z-10 w-[98vw]">
